 ...
 */
package com.device.cockpits.setup;
...
import com.device.cockpits.constants.DeviceCockpitsConstants;
 ...
 */
@SystemSetup(extension = DeviceCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(DeviceCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(DeviceCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						DeviceCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/devicecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
